...
from king.whois import run_whois  # ✅ WHOIS
from king.igris import ask_igris  # ✅ Import AI brain
...
async def ai_auto_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Group chat → respond only if trigger words or reply to bot
...
# Helper function to send messages with "Copy Code" button
async def send_copyable_message(update: Update, text: str):
    """
    Sends a message with a copy button under code blocks.
    Only the code inside ``` ``` is copied, not explanations.
    """
    markup = None
    ...
    if code_match:
        ...
        # Add copy button with only code
...
def main():
    ...
    # Commands
    summon_igris_handler = CallbackQueryHandler(
        summon_igris_callback, pattern="^arise$")
    ...
    # AI Auto-chat handler
    app.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND, ai_auto_chat))
